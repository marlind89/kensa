...
{
    private static final Logger logger = LoggerFactory.getLogger(FreskeJoinsVoiceChannelEventHandler.class);
    ...
    {
        return events.publish(shared -> shared
            ...
            .switchMap(event -> shared
                ...
                .onErrorResume(error ->
                {
                    logger.error("Error occurred while handling voice state update event", error);
                    return Mono.empty();
                })
            )
